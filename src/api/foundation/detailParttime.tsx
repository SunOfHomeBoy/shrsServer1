...
export default async function detailParttime(req: request, res: response, parameters: any): Promise<IResult> {
        ...
        let document: any = await schema.resume.findParttime(parameters.resumeID)
        if (!document) {
        ...
        if (parameters.openID !== parameters.resumeID) {
                ...
                // 判断购买记录是否过期
                let orderSafe = await schema.resume.checkSafe(parameters.openID, parameters.resumeID, 'PARTTIME')

                if (orderSafe) {
                        return render({ code: 200, msg: '', data: schema.resume.formatParttime(document, 'SAFE', orderSafe) })
                }

                await log.parttime(req, parameters.resumeID, parameters)
                return render({ code: 200, msg: '', data: schema.resume.formatParttime(document, 'UNSAFE') })
        }
        ...
        return render({ code: 200, msg: '', data: schema.resume.formatParttime(document, parameters.resumeStep) })
}