...
function paypassMember(req, res, parameters) {
            ...
            switch (_a.label) {
                case 0:
                    if (/^[0-9]{6}$/i.test(parameters.newPassword) === false || parameters.newPassword !== parameters.confirmPass) {
                        return [2, foundation_1.render({ code: 2019, msg: 'Invalid Parameters 0' })];
                    }
                    return [4, foundation_1.schema.member.findById(foundation_1.utils.openIDDecode(parameters.openID))];
                case 1:
                    memberInfo = _a.sent();
                    if (foundation_1.utils.empty(memberInfo)) {
                        return [2, foundation_1.render({ code: 2018, msg: 'Invalid Member Information' })];
                    }
                    if (memberInfo.passwordPay && memberInfo.passwordPay !== foundation_1.utils.cryptoPassword(String(parameters.oldPassword))) {
                        return [2, foundation_1.render({ code: 2019, msg: 'Invalid Parameters' })];
                    }
                    memberInfo.passwordPay = foundation_1.utils.cryptoPassword(parameters.newPassword);
